...
const suggestionLst = document.getElementById('suggestions') as HTMLInputElement
...
function postAndUpdate(): void {
  ...
  //  HINT: use .innerHTML
  suggestionLst.innerHTML = ""
  // TODO: add a type annotation to make this of type MatchesRequestDatacontent : MatchesRequest
...
function updateSuggestions(matches: string[]): void {
  ...
  for (const match of matches) {
    suggestionLst.innerHTML += `<li tabindex="0">${match}</li>`;
  }
}
...
document.addEventListener("keyup", async (event) => {
  if (event.key === "1") {
    await updateValues("Aries", "Gemini", "Libra");
    postAndUpdate();
  }
});
